use <hardware.scad> 
use <../../MultiWiiDrone/Hardware/MultiWiiBoardSupport.scad>
...
module controller(hole = false)
{
    if (!hole)
    {
        translate([-7/2, -7/2, 1.3]) scale(0.1) plate1(temper = true);
    }
    
    if (hole)
    {
        translate([-7/2, -7/2, 0]) cube([7, 7, 4]);
        translate([0, -7/2 + 0.75, -1.5]) cylinder(d = 1, h = 3.5); 
        translate([0, 7/2 - 0.75, -1.5]) cylinder(d = 1, h = 3.5); 
        translate([7/2 - 0.75, 0, -1.5]) cylinder(d = 1, h = 3.5); 
        translate([-7/2 + 0.75, 0, -1.5]) cylinder(d = 1, h = 3.5); 
    }
}
...
module wing_support(length = 10, depth = 0.3, offset = 1.5, hole = false)
{
    ...
    y = h * (a + 2) / (l + 3) - (h + depth);
    x = l * (y + 1) / (depth - h) + a + 2.5;
   
    points=[
        [0,depth*2], [1, depth*2], [1, 0],
        [l + 1, 0], [l + 1, depth], [length - a - 1, depth], [length - a - 1, 0],
        [l + 4, 0], [l + 4, -depth], [a + 3, y],
        [a + 3, -h - 4], [a + 1, -h - 4], [a - 0.5, -h],
        [1, -h], [1, -(h + depth)], [0, -(h + depth)]];
    points2=[
        [0.5, -0.5], [l - 0.3, -0.5], [x, -0.5 - depth], 
        [a + 2.5, y + 0.5],
        [a + 2.5, -h - 4 + 0.5], [a + 1.4, -h - 4 + 0.5],
        [a - 0.1, -h + 0.5],
        [0.5, -h + 0.5]];

    color("LightGrey", 0.5) translate([0, (w - depth)/2, 0])
    {
        if (!hole) difference()
        {
            union()
            ...
            translate([length - a - 1.5 - 0.3/2, -0.1, -v + depth*2 - 0.1]) cube([0.3, depth+0.2, v]);
            translate([length - a - 1.5 - 0.6/2, -0.1, -v + depth*2 + 0.1]) cube([0.6, depth+0.2, 0.3]);
        }
        
        if (hole)
        {
            translate([0, 0, -(h + 1.5)]) cube([1, depth, h + 3]);
            translate([length - a - 2, 0, 0]) cube([1, depth, depth + 0.1]);
        }
    }
}
    ...
    translate([a, -w/2, h])
    {
        if (!hole) difference()
        {
            color("Yellow") union()
            {
                ...
                translate([length - a, w/2, 0]) cylinder(d = w*1.4, h = depth);
            }
            
            wing_support(length, depth, offset, hole = true);
            
            // screws
            translate([0.5, 0.5, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
            translate([0.5, w-0.5, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
            translate([0.5, w/2, 0.15]) cube([0.3, 0.3, depth + 0.2], center= true);
            translate([length - a - 1.5, w/2, 0.15]) cube([0.3, 0.3, depth + 0.2], center= true);
            
            // engine
            translate([length - a, w/2, -0.1]) cylinder(d = 1, h = depth + 0.2);
            translate([length - a - 0.672, w/2 - 0.672, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
            translate([length - a + 0.672, w/2 + 0.672, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
            translate([length - a - 0.565, w/2 + 0.565, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
            translate([length - a + 0.565, w/2 - 0.565, -0.1]) cylinder(d = 0.3, h = depth + 0.2);
        }
    
        if (support)
        {
            wing_support(length, depth, offset, hole);
        }
        
        if (hole)
        {
            translate([0.5, 0.5, -h-1.5]) cylinder(d = 0.3, h = h + 3);
            translate([0.5, w-0.5, -h-1.5]) cylinder(d = 0.3, h = h + 3);
        }
    }
}
...
module govern()
{
    color("Yellow") scale([1.025, 1, 1.025]) translate([0.6, 0.3/2, -0.3]) rotate([90, 0, 0]) scale([0.2, 0.2, 1]) linear_extrude(height = 0.3) import("govern.svg", convexity=3);
}

module cabin()
{
    color("Yellow") translate([-1, 0.3/2, -3.2]) rotate([90, 0, 0]) linear_extrude(height = 0.3) scale([0.8, 0.8, 1]) import("cabin.svg", convexity=3);
}
...
module lower_plate()
{
    color("Yellow") difference()
    {
        translate([0, 0, 0]) plate(bottom = 1, depth = 0.3);
        translate([-l, 0, 0.3]) rotate([0, 0, 90]) wing(length = l + e, depth = 0.3, hole = true);
        translate([-l, 0, 0.3]) rotate([0, 0, -90]) wing(length = l + e, depth = 0.3, hole = true);
        translate([0, 0, 0.3]) rotate([0, 0, 45]) wing(length = f + sqrt(e*e*2), depth = 0.3, hole = true);
        translate([0, 0, 0.3]) rotate([0, 0, -45]) wing(length = f + sqrt(e*e*2), depth = 0.3, hole = true);
...
module main()
{
    ...
    color("LightGrey", 0.5) translate([0, 0, 0.3]) rotate([0, 0, 45]) wing(length = f + sqrt(e*e*2), depth = 0.3);
    color("LightGrey", 0.5) translate([0, 0, 0.3]) rotate([0, 0, -45]) wing(length = f + sqrt(e*e*2), depth = 0.3);
...
module schema()
{
    projection($fn=40)
    {
        ...
        translate([0, 10, 0]) lower_plate();
        
        translate([0, 18, 0]) rotate([90, 0, 0]) wing_support(length = l + e, depth = 0.3);
        translate([-2, 18, 0]) rotate([90, 180, 0]) wing_support(length = l + e, depth = 0.3);
        translate([0, 28, 0]) rotate([90, 0, 0]) wing_support(length = f + sqrt(e*e*2), depth = 0.3);
        translate([-2, 28, 0]) rotate([90, 180, 0]) wing_support(length = f + sqrt(e*e*2), depth = 0.3);
        
        translate([-2, 38, 0]) wing(length = l + e, depth = 0.3, support = 0);
        translate([-15, 38, 0]) wing(length = l + e, depth = 0.3, support = 0);
        translate([-2, 43, 0]) wing(length = f + sqrt(e*e*2), depth = 0.3, support = 0);
        translate([-20, 43, 0]) wing(length = f + sqrt(e*e*2), depth = 0.3, support = 0);
    }
}
